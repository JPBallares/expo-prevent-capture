...
export async function isAvailableAsync(): Promise<boolean> {
  return (
    !!ExpoPreventCaptureModule.preventScreenCapture &&
    !!ExpoPreventCaptureModule.allowScreenCapture
  );
}
...
export async function preventScreenCaptureAsync(
  ...
  if (!ExpoPreventCaptureModule.preventScreenCapture) {
    throw new UnavailabilityError("ScreenCapture", "preventScreenCaptureAsync");
  }

  if (!activeTags.has(key)) {
    activeTags.add(key);
    await ExpoPreventCaptureModule.preventScreenCapture();
  }
}
...
export async function allowScreenCaptureAsync(
  ...
  if (activeTags.size === 0) {
    await ExpoPreventCaptureModule.allowScreenCapture();
  }
}
...
export async function getPermissionsAsync(): Promise<PermissionResponse> {
  if (ExpoPreventCaptureModule.getPermissionsAsync) {
    return ExpoPreventCaptureModule.getPermissionsAsync();
  }
...
export async function requestPermissionsAsync(): Promise<PermissionResponse> {
  if (ExpoPreventCaptureModule.requestPermissionsAsync) {
    return ExpoPreventCaptureModule.requestPermissionsAsync();
  }
  return defaultPermissionsResponse;
}
